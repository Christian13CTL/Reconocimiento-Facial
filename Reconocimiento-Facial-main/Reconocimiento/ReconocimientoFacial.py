...
#lee el modelo
face_recognizer.read("modeloLEigenFace.xml")
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if result[1] < 90000: #los valores mas bajos o cercanos a cero quiere decir que el rostros tiene similitud a los entrenados
                    nombre =peopleList[result[0]]
                    leer_nombre(nombre)
                    cv2.putText(frame, '{}'.format(peopleList[result[0]]), (x, y-25), 2, 1.1, (0, 255, 0), 1, cv2.LINE_AA)
                    cv2.rectangle(frame, (x,y), (x + w, y + h),(0, 255, 0),2)
        else:
                    leer_nombre('Desconocido')
